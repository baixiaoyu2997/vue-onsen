<template>
    <v-ons-page class="searchPage" :infinite-scroll="loadMore">
        <div class="content">
            <div class="header">
                <v-ons-search-input type="search" v-model="searchValue" placeholder="搜索电影 / 电视剧 / 影人" class="search" @keyup.13="doSearch" transparent>
                    ...
                    <v-ons-row class="searchMovieList">
                        <v-ons-row v-for="item in searchMovieList" :key="item.id" class="searchMovieList__item" :id="item.id" @click="toDetailPage(item.id)">
                            ...
                            <v-ons-col class="item__right">
                                ...
                                <div v-if="item.directors.length>0">
                                    导演：{{item.directors["0"].name}}
                                </div>
                                <div v-if="item.casts.length>0">主演：
                                    <span v-for="(item,index) in item.casts" :key="index">{{index!=0?" / ":""}}{{item.name}}</span>
                                </div>
                            </v-ons-col>
...
export default {
    ...
    methods: {
        loadList(done) {
            ...
            this.$http.jsonp(that.url + that.searchValue + "&start=" + that.start * 20 + "&count=" + that.count).then((response) => {
                that.start += 1;
                that.searchMovieList = that.searchMovieList.concat(response.body.subjects);
