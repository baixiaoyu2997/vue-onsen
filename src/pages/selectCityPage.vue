<template>
  <v-ons-page class="selectCityPage">
    <v-ons-toolbar modifier="noshadow">
      <div class="left">
        <v-ons-back-button class="back-button">
        </v-ons-back-button>
      </div>
    ...
    <div class="middleBlock">
      ...
      <v-ons-row class="hotCityItem">
        <v-ons-col @click="selectCity(item.name)" width="28.9%" v-for="(item,index) in hotCity" :key="item.id" :class="index>11?'noMargin':''">{{item.name}}</v-ons-col>
      </v-ons-row>
    </div>
...
<script>
  ...
  export default {
    ...
    computed: {
      hotCityUrl() {
        return this.$root.commonURL + "/v2/loc/list"
      },
      positionCity() {
        return this.$store.state.selectPage.city.trueValue && this.$store.state.selectPage.city.value
      }
    },
    methods: {
      getCity() {
      ...
      renderItem(i) {
        const that = this;
        return new Vue({
          template: '<v-ons-list-item :key="index" @click="selectCity2(' + i + ')" modifier="longdivider">' +
            that.getListItem(i) +
            "</v-ons-list-item>",
          data() {
            return {
      ...
      getListItem(i) {
        ...
        if (list[i].first) {
          border = "";
          listItem =
            "<v-ons-row class='listRowHeader' id='header" +
            (i + 1) +
            "'>" +
            list[i].key +
            "</v-ons-row>";
