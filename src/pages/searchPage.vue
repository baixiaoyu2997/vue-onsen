<template>
    <v-ons-page class="searchPage">
        <div class="content">
            <div class="header">
                <v-ons-search-input type="search" v-model="searchValue" placeholder="搜索电影 / 电视剧 / 影人" class="search" @keyup.13="doSearch">
                    <input class="search-input">
                </v-ons-search-input>
                <v-ons-icon v-if="searchValue" @click="searchValue=''" icon="ion-ios-close-empty,material:md-close" size="2x" class="clearSearch"></v-ons-icon>
                <v-ons-button @click="back" modifier="quiet" ripple="false">取消</v-ons-button>
            </div>
            <div class="body">
                <v-ons-row v-if="!searchValue && searchHistoryArr && searchHistoryArr.length>0" class="searchHistory">
                    ...
                    <div class="searchHistory__main">
                        <v-ons-row class="searchHistory__item">
                            <v-ons-col :width="searchHistoryArr.length==1?'100%':'50%'" v-for="(item,index) in searchHistoryArr" :key="index" @click="quickSearch(item)">{{item}}</v-ons-col>
                        </v-ons-row>
                    </div>
                </v-ons-row>
                <v-ons-row v-if="searchMovieList.length>0" class="searchMovie">
                    ...
                    <v-ons-row class="searchMovieList">
                        <v-ons-row v-for="item in searchMovieList" :key="item.id" class="searchMovieList__item" :id="item.id" @click="toDetailPage(item.id)">
                            <v-ons-col width="60px" class="item__left">
                                <img :src="item.images.small" />
                            </v-ons-col>
                            <v-ons-col class="item__right">
                                <div v-html="movieName(item.title)" class="movieName"></div>
                                <div class="movieDetail">{{item.rating.average==0?"暂无评":item.rating.average}}分/{{item.year}}</div>
                            </v-ons-col>
...
export default {
    data() {
        return {
            url: this.$root.commonURL + "/v2/movie/search?q=",
            searchValue: "",
            searchMovieList: [],
    ...
    methods: {
        loadList(v) {
            if (v == "") return;
            let that = this;
            this.$http.jsonp(that.url + (v)).then((response) => {
        ...
        movieName(v) {
            return "<div>" + (v.indexOf(this.searchValue) > 0) ? v.replace(new RegExp(this.searchValue, "gm"), '<span>' + this.searchValue + '</span>') : v + "</div>"
        },
        back() {
        ...
        saveSearch() {
            ...
            this.searchHistoryArr = JSON.parse(localStorage.getItem("searchHistoryArr"));
        },
        quickSearch(v) {
            this.searchValue = v;
        },
