      ...
      <v-ons-row class="detailPage__main">
        ...
        <v-ons-row>
          ...
          <ons-row class="main__scroll-x">
            <ons-col width="84px" v-for="item in res.directors" :key="item.id" v-if="item.avatars" class="main__directors">
              <img v-if="item.avatars" :src="item.avatars.medium">
              <div class="g-overflowHide">{{item.name}}</div>
              <div class="scroll-x__label">导演</div>
            </ons-col>
            <ons-col width="84px" v-for="item in res.casts" :key="item.id" v-if="item.avatars" class="main__casts">
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        url: this.$root.commonURL + "/v2/movie/subject/",
        res: {},
        posterBG: "",
    ...
    created() {
      var that = this;
      this.$http.jsonp(this.url + this.id).then((response) => {
        let res = response.body;
        this.res = res;
        RGBaster.colors(res.images.large, {
          paletteSize: 200,
          exclude: ['rgb(0,0,0)', "rgb(255,255,255)"],
          success: function (payload) {
            that.posterBG = payload.palette[199];
          }
        });
        this.flag = true;
      }, (err) => {
