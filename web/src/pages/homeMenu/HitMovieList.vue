<template>
  <v-ons-page :infinite-scroll="loadMore">
    <div v-show="items.length>0" class="content">
      ...
      <v-ons-list>
        <v-ons-list-item v-for="item in items" :key="item.id" :id="item.id" modifier="longdivider" size="contain" tappable @click="toDetailPage(item.id)">
          <div class="left">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      loadList(s, done) {
        let that = this;
        this.$http.jsonp(that.url + that.city + "&start=" + (s || that.start * 20) + "&count=" + that.count).then((
          response) => {
